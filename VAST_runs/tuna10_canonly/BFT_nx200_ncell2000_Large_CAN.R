...
surveys <- surveys[surveys$location =='can',]

# Remove points not in model domain (on land)
region_shape <- st_read(here('Data/GIS/CanadaEEZ.shp'))
...
region_shape<- st_read(here::here("Data/GIS/CanadaEEZ.shp"))
region_shape <- as(region_shape, 'Spatial')
region_shape <- st_as_sf(spTransform(region_shape, CRS("+init=epsg:4326")))
#region_shape <- st_transform(region_shape, "EPSG:4326")
region_shape <- st_make_valid(region_shape)
...
us <- st_read(here('Data/GIS/CanadaEEZ.shp'), quiet=T)
us <- as(us, 'Spatial')
us <- st_as_sf(spTransform(us, CRS("+init=epsg:4326")))
#us <- st_transform(us, st_crs(coast))
us$Region <- 'Canada'
us <- dplyr::select(us, -OBJECTID, -Id, -Shape_Leng, -Shape_Area)
...
strata_use<- data.frame(STRATA = c("Canada"))
...
working_dir <- here::here("VAST_runs/tuna10_canonly")
...
save.image('tuna10_canfirst.RData')
