...
#### Run VAST ####
setwd(here('VAST_runs/tuna9'))
# Set working directory
working_dir <- here::here('VAST_runs/tuna9')
...
# Test correlation
df_cormat <- dplyr::select(covars, sst, depth, slp, nao, amo, prey)
...
rm(list=setdiff(ls(), c("scaled.covars", "settings", 'survs',
                        'grid_NWA_BTS')))
gc()
...
# Model formula
hab_formula <- ~ bs(sst, degree = 3, intercept = FALSE) + 
