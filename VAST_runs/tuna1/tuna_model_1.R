...
colnames(survs) <- c('Category', 'Response_variable', 'Effort', 'Year', 'Lon', 'Lat')
...
fit = fit_model( 
  ...
  Lon_i = survs[,'Lon'], 
  t_i = survs[,'Year'],
  b_i = as_units(survs[,'Response_variable'], 'counts'),
  a_i = survs[,'Effort'],
  c_iz = survs[,'Category'],
