...
dat <- read.csv(here('Data/Clean/LPS_SmallTarget_TripLevel.csv'))
# Remove duplicates
dat <- unique(dat)
...
target_bft=subset(target_bft,
                  prim1 %in% c(4677, 4678, 4672)|
                  prim2 %in% c(4677, 4678, 4672))
...
# Add fixed lat-lon dataframe
fixll <- read.csv(here('Data/Clean/LPS_compiled_0222_bfttargeted_allsizes.csv'))
fixll <- dplyr::select(fixll, id, latddmm, londdmm)
fixll <- unique(fixll)
target_bft$londdmm <- NULL; target_bft$latddmm <- NULL

# Merge
target_bft <- left_join(target_bft, fixll, by=c('id'))

# Remove observations without spatial data
target_bft <- target_bft[!is.na(target_bft$londdmm) &
                         !is.na(target_bft$latddmm),]
...
nwat <- st_transform(nwat, st_crs(bft_sf))
# Intersect
good <- st_intersection(bft_sf, nwat)
# Remove stations on land
...
write.csv(target_bft,
          here('Data/Clean/LPS_SmallTarget_Clean_2024_2.csv'),
          row.names = F)
