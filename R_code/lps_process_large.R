...
catch_data <- dplyr::select(
  catch_data, id, year, month, stcode, catch_n
)
...
catch_data[catch_data$id %notin% target.main$id,]
...
names(disposition) <- c('Whole', '1', 'Gutted', 'Headed', 'Skip')
...
write.csv(dat2,
          here('Data/Clean/LPS_LargeTarget_TripLevel.csv'),
          row.names = F)
