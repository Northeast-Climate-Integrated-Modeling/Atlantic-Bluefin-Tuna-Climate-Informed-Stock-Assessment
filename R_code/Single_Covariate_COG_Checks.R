...
n.spring <- ggplot() +
  ...
  ggtitle('Center of gravity northing - single covariate models') +
...
e <- ggplot() +
  ...
  ggtitle('Center of gravity easting - single covariate models') +
  theme(legend.position = 'bottom',
        legend.text = element_text(size=6),
        legend.title = element_text(size=8)) +
  guides(color = guide_legend(nrow = 1))

ggsave(filename=paste0(here('VAST_runs/tuna13_usonly/natural_splines/SingleCovs', '/cogshifts_northing.png')),
       n, width = 5.5, height=4.25, units='in')

ggsave(filename=paste0(here('VAST_runs/tuna13_usonly/natural_splines/SingleCovs', '/cogshifts_easting.png')),
       e, width = 5.5, height=4.25, units='in')
...
ggplot() +
  geom_sf(data=coast, col=NA) +
  geom_sf(data=gom, fill=NA) +
  #geom_sf(data=re.lin, aes(col=Model),
  #        lwd=0.3) +
  geom_sf(data=re.sf[re.sf$Year %in% c(1982,2021),],
         aes(col=Year)) +
  facet_wrap(vars(Model, Season)) +
