wd='L:/HMS data/LPS/LPS_20190322/compiled_data/'
setwd(wd)
dat=read.csv('LPS_trip_level_0218.csv')
head(dat)


## LARGE MEDIUM AND GIANT ANALYSIS DATASET 

# FILTER FOR BLUEFIN TARGETED TRIPS, JULY to OCTOBER, PRIVATE AND CHARTER VESSELS ONLY, FISHED BETWEEN 1 and 24 HRS 
filter_bft=subset(dat,month%in%c(7:10)&prim_op%in%c(1,2)&stcode%in%c(9,23,25,33,34,36,44,51)&fhours>0.99&fhours<24.01)
large_bft=subset(filter_bft,prim1%in%c(4671,4679)|prim2%in%c(4671,4679))
...
write.csv(with(large_bft,cbind(id,year,month,day,stcode,ppstate,latddmm,londdmm,prim1,prim2,prim_op,todayt,category,tourn,miles,depth,sst,
	hook,lines,fhours,bt_live,bt_art,bt_dead,fm_troll,fm_chunk,fm_chum,fm_other,closure,large_med_bft,giant_bft))
		,"lgmd_giant_bft_analysis_0218.csv",row.names=FALSE)


### SMALL BFT ANALYSIS DATASET

# FILTER FOR TARGETED TRIPS, JUNE to SEPTEMBER
filter_bft_2=subset(dat,month%in%c(6,7,8,9)&stcode%in%c(9,10,24,25,34,36,44,51)&prim_op%in%c(1,2)&fhours>0&fhours<24)
...
write.csv(school_bft,"school_bft_analysis_0218.csv")
...
write.csv(large_school_bft,"large_school_bft_analysis_0218.csv")
